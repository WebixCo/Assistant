...
let createNote = (noteTitle, noteBody) => {
  ...
  a.setAttribute("onclick", "displayNote(this.textContent.split('✕')[0], this.textContent.split('✕')[1])")

  li.appendChild(a);
  ul.appendChild(li);
};
function displayNote(title, cont){
  document.getElementById("selNttl").textContent=title
  document.getElementById("selNtxt").textContent=cont
}
